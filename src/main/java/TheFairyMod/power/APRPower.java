...
public class APRPower extends AbstractPower {

    public static final String POWER_ID = TheFairyMod.makeID("APRPower");
    ...
    public APRPower(final AbstractCreature owner, final int amount) {
        name = NAME;
        ID = POWER_ID;
        this.owner = owner;
        this.amount = amount;
        ...
        if (PREVIOUS_AMT < amount) {
            for (AbstractCard c : AbstractDungeon.player.hand.group) {
                if (c.hasTag(CustomTags.REQUIRES) && c.hasTag(CustomTags.BULLET)) {
                    c.baseDamage = c.baseDamage + amount;
                    PREVIOUS_AMT = amount;
                }
            }
